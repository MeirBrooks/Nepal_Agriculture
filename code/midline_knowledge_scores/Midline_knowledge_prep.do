...
tempfile baseline_all baseline_t baseline_g baseline_f midline_all
...
merge 1:1 `id_vars' using `baseline_t', keep(match) nogen
save `knowledge_TOMATO', replace

/// merge ginger
use `midline_test_GINGER', clear
merge 1:1 `id_vars' using `baseline_g', keep(match) nogen
save `knowledge_GINGER', replace

/// merge french beans
use `midline_test_FRENCH_BEANS', clear
merge 1:1 `id_vars' using `baseline_f', keep(match) nogen
